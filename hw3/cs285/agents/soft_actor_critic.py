...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            result1, result2 = next_qs.split(1, dim=0)
            next_qs = torch.stack((result2, result1), dim=0)
        elif self.target_critic_backup_type == "min":
            min_result = next_qs.min(dim=0).values
            next_qs = min_result.repeat(self.num_critic_networks ,1)
            # raise NotImplementedError
        elif self.target_critic_backup_type == "mean":
            mean_result = next_qs.mean(dim=0)
            next_qs = mean_result.repeat(self.num_critic_networks , 1)
            # raise NotImplementedError
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                try:
                    next_action_entropy = next_action_entropy.reshape(next_qs.shape)
                except:
                    next_action_entropy = next_action_entropy.sum(dim = -1).reshape(next_qs.shape)
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        """
        if isinstance(action_distribution, torch.distributions.Independent):
            action_distribution = action_distribution.base_dist.base_dist
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            expand_obs = obs.unsqueeze(0).expand(self.num_actor_samples, *obs.shape)
            q_values = self.critic(expand_obs, action)
            ...
            q_values = torch.mean(q_values, axis=0).detach() 
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        # Note: Think about whether to use .rsample() or .sample() here...
        action = action_distribution.rsample((self.num_actor_samples, )) #N, BS, ac_dim

        # TODO(student): Compute Q-values for the sampled state-action pair
        expand_obs = obs.unsqueeze(0).expand(self.num_actor_samples, *obs.shape)
        q_values = self.critic(expand_obs, action) # num_c, N, BS

        # TODO(student): Compute the actor loss
        min_q_values = torch.min(q_values, dim=0).values # (BS,)
        loss =  min_q_values.mean()  # return mean?
