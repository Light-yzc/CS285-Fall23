...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            values = self.critic(obs_tensor).squeeze(dim=1)
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    # pass
                    # print(type(terminals), type(values), type(rewards))
                    delta = rewards[i] + self.gamma * values[i+1] * (1 - terminals[i]) - values[i]
                    advantage = delta + self.gamma * self.gae_lambda * advantages[i+1] * (1 - terminals[i])
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i, r in enumerate(rewards):
            total_reward += pow(self.gamma, i) * r
        out = []
    ...
    #         for j, r in enumerate(chunk_rewards):
    #             tmp_reward += pow(self.gamma, j) * r
    #         out.append(tmp_reward)
    #     # print(out)
    #     return out
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> np.ndarray:
        ...
        q = np.zeros(T, dtype=np.float32)
        running = 0.0
        for t in range(T - 1, -1, -1):
            running = r[t] + self.gamma * running
            q[t] = running
        return q